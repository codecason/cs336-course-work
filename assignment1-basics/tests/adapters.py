...
def run_rope(
    ...
    """
    return rope(in_query_or_key, token_positions, theta, max_seq_len)
...
def run_get_batch(
    ...
    y = torch.stack([
        torch.from_numpy(dataset[i + 1 : i + 1 + context_length]).long()
        for i in ix.tolist()
    ])
...
def run_cross_entropy(
    ...
    inputs -= c
    logsumexp = torch.log(torch.exp(inputs).sum(dim=-1, keepdim=True))
...
def run_load_checkpoint(
    ...
    """
    # Load the checkpoint dict. torch.load accepts both paths and file-like objects.
    # Implicitly, our previous save used standard pickle, so we load with the same
    # format (weights_only defaults differ across PyTorch versions; set it explicitly
    # to False to safely restore the full optimizer state dict).
    checkpoint = torch.load(src, weights_only=False)

    # Restore each piece of serialized state.
    model.load_state_dict(checkpoint["model"])
    optimizer.load_state_dict(checkpoint["optimizer"])

    # Return the previously-serialized iteration count.
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    """
    if kwargs:
        raise TypeError(f"unsupported training options: {sorted(kwargs)}")
